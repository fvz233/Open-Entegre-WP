...
{
    public const CRON_HOOK = 'multi_sync_stock_push_event';
    ...
    public static function init()
    {
        ...
        add_action(self::CRON_HOOK, array(__CLASS__, 'run_scheduled_stock_push'), 10, 1);
    ...
    public static function run_scheduled_stock_push($supplier_id)
    {
        $supplier_id = (int) $supplier_id;
        if ($supplier_id <= 0) {
            return;
        ...
        if (is_wp_error($result)) {
            ...
            $log_model->log(
                ...
                'stock_push',
                'error',
                '[cron] Stok push kuyruğa eklenemedi: ' . $result->get_error_message()
            );
        }
    }
    ...
    private static function should_schedule($supplier, $settings)
    {
        ...
        $mode = self::sanitize_stock_automation_mode(
            isset($settings->stock_automation_mode) ? $settings->stock_automation_mode : 'scheduled'
        );
        if ($mode !== 'scheduled') {
            return false;
        }
    ...
    private static function sanitize_stock_automation_mode($mode)
    {
        $value = sanitize_key((string) $mode);
        if (!in_array($value, self::ALLOWED_STOCK_AUTOMATION_MODES, true)) {
            return 'scheduled';
        }
        return $value;
    }
